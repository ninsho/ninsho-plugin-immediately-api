  ...
  private async method(
    ...
    const req = {
      ...
      options: {
        ...
        mailFormat: {
          subject: options?.mailFormat?.subject ?? mailFormat.ChangePasswordCompleat.subject,
          body: options?.mailFormat?.body ?? mailFormat.ChangePasswordCompleat.body,
        },
    ...
    if (session.fail()) /* istanbul ignore next */ return session.pushReplyCode(2243)
    if (session.response.m_role < req.options.rolePermissionLevel) return new E403(2244)
    if (session.response.m_status != MStatus.ACTIVE) return new E403(2245)
    ...
    if (req.options.hooks) {
      ...
      if (res.fail()) return res.pushReplyCode(2246) as any
    }
    ...
    /* istanbul ignore if */
    if (upd.fail()) return upd.pushReplyCode(2248)
    ...
    if (req.options.hooks) {
      ...
      if (res.fail()) {
        await lend.modules.pool.rollbackWithRelease(connection)
        return res.pushReplyCode(2251) as any
      }
    }
